...
<script type="text/javascript" charset="utf-8">
	$(document).ready(function() {
		...
		$('#CarmodelCarbrandId').change(function(){
			$('#CarmodelAdminListmodelsForm').submit();
		});
	} );
</script>
		...
		<div class="bs-example table-responsive">
		  <table class="table table-striped table-bordered table-hover" id="modeltable">
			...
			<tr>
				...
				<td class="actions">
					<?php echo $this->Html->link($this->Html->url(array('controller' => 'testdrives', 'action' => 'landingpage', 'admin' => false, AuthComponent::user('id'), $carmodel['Carmodel']['id']), true)); ?>
				</td>
			</tr>
			<?php endforeach; ?>
			 </tbody>
