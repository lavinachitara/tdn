...
<script type="text/javascript" charset="utf-8">
	$(document).ready(function() {
		...
		$('#TestdriveCarbrandId').change(function(){
			$('#TestdriveAdminIndexForm').submit();
		});
	} );
</script>
		...
		<div class="bs-example table-responsive">
		  <table class="table table-striped table-bordered table-hover" id="testdrivetable">
			...
			<tr>
				...
				<td><?php 
				//echo h($testdrive['Testdrive']['scheduled']?'Yes':'No');
				$str = "";
				$status = 0;
				if($testdrive['Testdrive']['scheduled'])
				{
					$str = "Yes";
				}else{
					$status = 1;
				...
				if(AuthComponent::user('isAdmin'))
					...
					echo $this->Html->link(__($str), array('action' => 'admin_setstatus', $testdrive['Testdrive']['id'], $status, 'scheduled'));
				?>&nbsp;</td>
				<td><?php 
				//echo h($testdrive['Testdrive']['scheduled']?'Yes':'No');
				$str = "";
				$status = 0;
				if($testdrive['Testdrive']['bought'])
				{
					$str = "Yes";
				}else{
					$status = 1;
				...
				if(AuthComponent::user('isAdmin'))
					...
					echo $this->Html->link(__($str), array('action' => 'admin_setstatus', $testdrive['Testdrive']['id'], $status, 'bought'));
				?>&nbsp;</td>
