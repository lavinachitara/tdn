...
class PublishersController extends AppController {
	...
	public function admin_index() {
		...
		$conditions = array('User.isAdmin' => 0, 'User.isDealer' => 0);
	...
	public function admin_view($id = null) {
		...
		$options = array('conditions' => array('User.' . $this->User->primaryKey => $id, 'User.isAdmin' => 0, 'User.isDealer' => 0));
	 ...
	 * @return void
	 */
	public function admin_changestatus($id, $status = 0) {
		$this->User->id = $id;
		...
		$options = array('User.' . $this->User->primaryKey => $id, 'User.isAdmin' => 0, 'User.isDealer' => 0);
			
		if($status)
			$msg = "The publisher has been activated";
		else
			$msg = "The publisher has been deactivated";

		if($this->User->updateAll(array('User.isActive' => $status), $options))
		{
			$this->Session->setFlash(__($msg));
		}
		else {
			$this->Session->setFlash(__('The publisher could not be changed. Please, try again.'));
		}
